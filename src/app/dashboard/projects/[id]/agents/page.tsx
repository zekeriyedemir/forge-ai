...
export default async function Agents({ params }: { params: Promise<{ id: string }> }) {
  ...
  const runs = await db.agentRun.findMany({ where: { projectId: id }, orderBy: { createdAt: "desc" }, include: { events: { orderBy: { createdAt: "asc" } } } });
  return <section><h2 className="text-2xl font-semibold">Agent executions</h2><p className="mt-2 text-sm text-slate-400">Every step has a persisted run, status, and event history.</p><div className="mt-6 space-y-4">{runs.map(run => <article key={run.id} className="rounded-2xl border border-white/10 bg-[#111824] p-6"><div className="flex justify-between"><h3 className="font-semibold capitalize">{run.type.toLowerCase()} Agent</h3><span className="text-xs text-orange-400">{run.status}</span></div><p className="mt-2 text-xs text-slate-500">Execution {run.id} · {run.provider ?? "pending"} / {run.model ?? "—"}</p>{run.error && <p className="mt-3 text-sm text-red-400">{run.error}</p>}<div className="mt-5 space-y-3">{run.events.map(event => <p key={event.id} className="border-l border-orange-400/40 pl-4 text-sm text-slate-300">{event.message}</p>)}</div></article>)}{runs.length===0 && <p className="rounded-2xl border border-dashed border-white/15 p-8 text-slate-500">No executions yet. Start the workflow from Overview.</p>}</div></section>;
}
